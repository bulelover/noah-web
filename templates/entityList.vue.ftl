<template>
	<div class="page-wrapper">
		<div class="white-box full-height">
			...
			<div class="search-flex mb16">
				...
				<div>
					<el-button v-if="$hasAuth('${permission!}-add')" type="primary" @click="add">新增</el-button>
				</div>
			</div>
		...
		<#if field_index gt 0>
			<#if field_index == 1>
				<el-table-column prop="${field.propertyName}" label="${(field.comment!?length gt 0)?then(field.comment,'未定义')}" min-width="120" show-overflow-tooltip>
					<template v-slot="{row}">
						<el-link type="primary" v-if="$hasAuth('${permission!}-view')"
	...
	<#------------  END 字段循环遍历  ---------->
          ...
          v-if="$hasAuth('${permission!}-edit') || $hasAuth('${permission!}-delete')">
					<template v-slot="{row}">
						<el-link v-if="$hasAuth('${permission!}-edit')" :underline="false" type="primary" @click="edit(row)">
							修改
						</el-link>
						<el-link v-if="$hasAuth('${permission!}-delete')" :underline="false" type="danger" @click="del(row)">
							删除
						</el-link>
					</template>
				</el-table-column>
