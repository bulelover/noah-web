...
public class XML {
	 ...
	 */
	public static JSONObject toJSONObject(String string, List<String> listKey) throws JSONException {
		return toJSONObject(string, false, listKey);
	}
	 ...
	 * @throws JSONException Thrown if there is an errors while parsing the string
	 */
	public static JSONObject toJSONObject(String string, boolean keepStrings, List<String> listKey) throws JSONException {
		return toJSONObject(new XmlJSONObject(), string, keepStrings, listKey);
	}
	 ...
	 * @since 5.3.1
	 */
	public static JSONObject toJSONObject(XmlJSONObject jo, String string, boolean keepStrings, List<String> listKey) throws JSONException {
		XMLTokener x = new XMLTokener(string, jo.getConfig());
		while (x.more() && x.skipPast("<")) {
			parse(x, jo, null, keepStrings, listKey);
		}
		return jo;
	}
	 ...
	 */
	private static boolean parse(XMLTokener x, XmlJSONObject context, String name, boolean keepStrings, List<String> listKey) throws JSONException {
		char c;
		int i;
		XmlJSONObject jsonobject;
		String string;
		String tagName;
		Object token;
			...
			if (c == '-') {
				...
				if ("CDATA".equals(token)) {
					if (x.next() == '[') {
						string = x.nextCDATA();
						if (string.length() > 0) {
							context.accumulate("content", string, listKey);
						}
						return false;
					}
				}
			...
			for (; ; ) {
				...
				if (token instanceof String) {
					...
					if (token == EQ) {
						...
						jsonobject.accumulate(string, keepStrings ? token : InternalJSONUtil.stringToValue((String) token), listKey);
						token = null;
					} else {
						jsonobject.accumulate(string, "", listKey);
					}
					...
					if (jsonobject.size() > 0) {
						context.accumulate(tagName, jsonobject, listKey);
					} else {
						context.accumulate(tagName, "", listKey);
					}
					return false;
					...
					for (; ; ) {
						...
						if (token == null) {
							...
							string = (String) token;
							if (string.length() > 0) {
								jsonobject.accumulate("content", keepStrings ? token : InternalJSONUtil.stringToValue(string), listKey);
							}

						} else if (token == LT) {
							// Nested element
							if (parse(x, jsonobject, tagName, keepStrings, listKey)) {
								if (jsonobject.size() == 0) {
									context.accumulate(tagName, "", listKey);
								} else if (jsonobject.size() == 1 && jsonobject.get("content") != null) {
									context.accumulate(tagName, jsonobject.get("content"), listKey);
								} else {
									context.accumulate(tagName, jsonobject, listKey);
								}
								return false;
